...
int command_input_handler(int sd, int events, void *arg) {
	...
	printf("Read %d bytes from command_wproc\n", ret);
	if (ret == 0) {
		printf("Command file worker seems to have died.\n");
	...
	while ((buf = iocache_use_delim(command_wproc->ioc, "\n", 1, &size))) {
		if (buf[0] == '[') {
			...
			buf[size] = 0;
			printf("%lu: master: Read raw external command '%s'\n", time(NULL), buf);
			}
...
static int command_file_worker(int sd) {
	...
	while(1) {
		...
		/* check for errors */
		if(pollval == -1) {
			printf("Failed to poll() command file pipe: %m\n");
			}
